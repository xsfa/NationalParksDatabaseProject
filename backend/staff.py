...
def getQuery(attribute, value, isCount):
    ...
    if not isCount:
        if attribute == "ssn" or attribute == "name" or attribute == "O_office_id" or attribute == "P_park_id":
            query = "SELECT name as Name, gender as Gender, CAST(salary as float) as Salary, super_ssn as Supervisor_SSN, O_office_id as Office_ID, P_park_id as National_Park_ID " \
            "FROM STAFF " \
            "WHERE {} = '{}'".format(attribute, value)

        elif attribute == "park_name":
            query = "SELECT S.name as Staff_Name, S.gender as Gender, CAST(S.salary as float) as Salary, S.super_ssn as Supervisor_SSN, S.O_office_id as Office_ID, park_name AS Park_Name "\
            ...
            "SELECT HS.S_ssn as SSN, S.name as Staff_Name, HT.trail_name as Overseeing_Area_Name " \
            "FROM HIKING_TRAIL_STAFF HS  " \
            "JOIN STAFF S ON HS.S_ssn = S.ssn " \
            "JOIN HIKING_TRAIL HT ON HS.H_trail_id = HT.trail_id " \
            "UNION " \
            "SELECT A.S_ssn as SSN, S.name as Staff_Name, A.Att_name as Overseeing_Area_Name " \
            "FROM ATTRACTION_STAFF A " \
            "JOIN STAFF S ON A.S_ssn = S.ssn " \
            "UNION " \
            "SELECT C.S_ssn as SSN, S.name as Staff_Name, C.CG_name as Overseeing_Area_Name " \
